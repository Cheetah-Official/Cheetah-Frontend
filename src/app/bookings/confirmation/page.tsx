  ...
  const onPayNow = async () => {
    ...
    try {
      ...
      if (url) {
        ...
        setInitError(
          "Failed to initialize payment: no authorization URL returned",
        );
      }
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-[#F6F6F6] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 max-w-xl w-full text-center">
        ...
        <p className="text-gray-600 mb-4">
          Thank you for booking with Cheetah.
        </p>
            ...
            <div className="text-sm text-gray-700">
              ...
              {date ? (
                <div className="text-gray-600">
                  Departure:{" "}
                  {new Date(decodeURIComponent(date)).toLocaleString()}
                </div>
              ) : null}
            </div>
          </div>
            ...
            {totals && (
              <div className="mt-2 text-sm text-gray-700">
                <div className="flex justify-between">
                  ...
                  <span>₦{Number(totals.base_amount).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span>Fees</span>
                  <span>₦{Number(totals.total_fees).toLocaleString()}</span>
                </div>
                <div className="flex justify-between font-semibold">
                  <span>Total</span>
                  <span>₦{Number(totals.grand_total).toLocaleString()}</span>
                </div>
              </div>
            )}
            {initError && (
              <div className="mt-2 text-xs text-red-600">{initError}</div>
            )}
          </div>
        )}
        ...
        <div className="text-sm text-gray-700 mb-6">
          ...
          <p className="mt-1">
            You can also find updates in your dashboard notifications.
          </p>
        </div>
        ...
        {!user && (
          <div className="mt-6 text-left bg-gray-50 border rounded-lg p-4">
            ...
            <p className="text-sm text-gray-600 mb-3">
              Securely access your tickets, payments, and trip updates from your
              dashboard.
            </p>
